---
title: 'Tools and References'
---

## Tools and References


**[Access coreBOS Issue Tracker](http://corebos.org/development)**

-   [coreBOS Issue Tracker](http://localhost/coreBOSDocumentation/others/devel/issue_tracker)
-   [Upgrade coreBOS 5.4 to coreBOS 5.5](http://localhost/coreBOSDocumentation/others/devel/upgradecb5402cb550)
-   [Upgrade vtiger CRM 5.4 to coreBOS 5.4](http://localhost/coreBOSDocumentation/others/devel/vt5tocb1)
-   [Migrate from vtiger CRM 6.0 to coreBOS](http://localhost/coreBOSDocumentation/others/devel/vt60tocb)
-   [Migrate from prototype/scriptaculous to jQuery](http://localhost/coreBOSDocumentation/others/devel/prototype2jquery)

## Technical Articles

-   [Development Tutorials](http://localhost/coreBOSDocumentation/developer-guide/development_framework/develtutorials)
-   [Webservice Development](http://localhost/coreBOSDocumentation/developer-guide/architecture-concepts/corebosws)
-   [Development Guidelines we try to adhere to](http://localhost/coreBOSDocumentation/developer-guide/architecture-concepts/developmentguidelines)
-   [How to setup a coreBOS development environment](http://localhost/coreBOSDocumentation/getting-started/develinstall)
-   [How to Anonymize your database](http://localhost/coreBOSDocumentation/developer-guide/development_framework/anonymizedb)
-   [Debugging](http://localhost/coreBOSDocumentation/developer-guide/development_framework/debuging)
-   [Javascript Debugging](http://localhost/coreBOSDocumentation/developer-guide/development_framework/jsdebuging)
-   [Commit Guidelines we try to adhere to](http://localhost/coreBOSDocumentation/developer-guide/architecture-concepts/commitguidelines)
-   [How to Contribute](http://localhost/coreBOSDocumentation/prologue/documentation)
-   [Bug Reports](http://localhost/coreBOSDocumentation/others/devel/bugreport)
-   [coreBOS Events, Links and Hooks](http://localhost/coreBOSDocumentation/developer-guide/development_framework/develtutorials/corebos_hooks)
-   [Developer Tips and Tricks](http://localhost/coreBOSDocumentation/developer-guide/development_framework/develtips)
-   [Database Structure](http://localhost/coreBOSDocumentation/developer-guide/development_framework/database)
-   [PDF Structure](http://localhost/coreBOSDocumentation/developer-guide/development_framework/pdfstructure)
-   [Helper Scripts](http://localhost/coreBOSDocumentation/developer-guide/development_framework/helperscripts)
-   [Packaging and Release Numbers](http://localhost/coreBOSDocumentation/developer-guide/architecture-concepts/packaging)
-   [coreBOS Updater](http://localhost/coreBOSDocumentation/developer-guide/development_framework/develtutorials/corebosupdater)
-   [Translating](http://localhost/coreBOSDocumentation/developer-guide/development_framework/translating)
-   [Logging stress test](http://localhost/coreBOSDocumentation/developer-guide/development_framework/stresstestlog)
-   [Calendar](http://localhost/coreBOSDocumentation/user-manual/calendar)
-   [Asterisk](http://localhost/coreBOSDocumentation/developer-guide/development_framework/asterisk)
-   [Menu Selection](http://localhost/coreBOSDocumentation/knowledge-base/menu-editor/menuselection)
-   [coreBOS Installation Process](http://localhost/coreBOSDocumentation/others/devel/installprocess)
-   [coreBOS Service Worker](http://localhost/coreBOSDocumentation/developer-guide/development_framework/serviceworker)
-   [Product/Price Enhancements](http://localhost/coreBOSDocumentation/developer-guide/architecture-concepts/cbproductprice)
-   [coreBOS Testing Project](http://localhost/coreBOSDocumentation/developer-guide/development_framework/testing)

## Vulnerabilities and Errorneous Changesets

-   [Vulnerabilities](http://localhost/coreBOSDocumentation/others/devel/vulnerabilities)
-   [Errorneous Changesets](http://localhost/coreBOSDocumentation/developer-guide/architecture-concepts/errorneouschangesets)

## Contributors


-   [Contributors](http://localhost/coreBOSDocumentation/others/contributors)

## Interesting Links

-   [License Woes](http://localhost/coreBOSDocumentation/getting-started/license/license)
-   [Contributor License Agreements](http://localhost/coreBOSDocumentation/prologue/cla)
-   [SVN &gt; GIT](http://localhost/coreBOSDocumentation/others/devel/svn2git)
-   [Cheat Codes for Good Documentation](http://sendgrid.com/blog/cheat-codes-for-good-documentation/)
